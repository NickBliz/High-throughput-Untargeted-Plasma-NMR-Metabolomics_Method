library(readr)
setwd("D:/MEGA/Publication 1/PILOT4 (2)/btp_final_3")
UF_PILOT4_3_new_25_PQN_min_noRSD0_3_GLOG <- read_csv("D:/MEGA/Publication 1/PILOT4 (2)/btp_final_3/bucket_table_final_UF.csv")
...
#write.csv(file="bucket_table_uf_new_20.csv", pearl)

#scale to MA peak (5.99)
captain=pearl
sum(colSums(is.na(captain)))
...
sparrow$`5.99000009`<- NULL
#write.csv(file="bucket_table_uf_new_20_MA.csv", sparrow)
...
cutler=beckett
#write.csv(file="bucket_table_uf_new_20_MA_PQN.csv", beckett)
...
#write.csv(final, file="bucket_table_uf_new_20_MA_PQN_GLOG.csv")

#MixOmics models
PLASMA_GLOG_forMIXOMIX = final
#row.names(cutler)=row.names(final)
#PLASMA_GLOG_forMIXOMIX = cutler
...
plotIndiv(pca.ENP, group = why, ind.names = FALSE,legend = T, title= 'PCA UF AMIX MA PQN GLOG')
#differentiation of groups in 3rd PC (2% explained var)
plotLoadings(pca.ENP, comp = 1, ndisplay = 30, title= 'PCA UF AMIX MA PQN GLOG')
...
mitch=vip(ENP.plsda)
write.csv(mitch, file="UF_AMIX_MA_PQN_GLOG_VIPs.csv")

#check for correlation with sequence (stability-degradation)
seq_check=match(row.names(ex), UF_PILOT4_3_new_25_PQN_min_noRSD0_3_GLOG$X1)
mitch=predict(ENP.plsda, ex)
predictions_uncorrected=as.matrix(mitch$class$mahalanobis.dist[,3])
...
save.image("D:/MEGA/Publication 1/PILOT4 (2)/btp_final_3/UF_AMIX_final_models.RData")